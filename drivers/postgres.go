...
func (db *Postgres) GetTables(database string) (tables map[string][]string, err error) {
	...
	rows, err := db.Connection.Query(fmt.Sprintf("SELECT table_name, table_schema FROM information_schema.tables WHERE table_catalog = '%s' ORDER BY table_name ASC", database))
	if err != nil {
		return tables, err
	}
